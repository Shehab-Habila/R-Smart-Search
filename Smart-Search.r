...
start_search <- function (string_to_find, string_or_vector_to_find_in, fuzzy = TRUE, char_range = 2) {
...
smart_search <- function (string_to_find, string_or_vector_to_find_in, min_word_matches = 0.3, fuzzy = TRUE, char_range = 2) {
  ...
  # GET THE MATCHES
  # matches_parameters <- list( c(wanted_word_index, ref_word_index, ref_item_index, letters_matched_percentage) )
  matches <- start_search(string_to_find, string_or_vector_to_find_in, fuzzy, char_range)
  ...
  for (counter in 1:num_final_scores) {
    ...
    texts   <- c(texts, string_or_vector_to_find_in[sorted_ref_indices[counter]])
